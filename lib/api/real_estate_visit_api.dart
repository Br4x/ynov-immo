...
class RealEstateVisitApi {
  ...
  RealEstateVisitApi([ApiClient apiClient])
      : apiClient = apiClient ?? defaultApiClient;

  /// get all real-estate-visit by pagination
  ///
  ///
  Future<List<RealEstateVisitPagination>> realEstateVisitGet(
      {List<String> where,
      String fields,
      String order,
      int offset,
    ...
    if (response.statusCode >= 400) {
      ...
      return (apiClient
          .deserialize(response.body, 'List<RealEstateVisitPagination>') as List)
          .map((item) => item as RealEstateVisitPagination)
          .toList();
    } else {
      return null;
    }
  ...
  Future<ApiResponse> realEstateVisitIdDelete(int id) async {
    ...
    // create path and map variables
    String path = "/real-estate-visit/{Id}"
  ...
  Future<RealEstateVisit> realEstateVisitIdGet(int id) async {
    ...
    if (response.statusCode >= 400) {
      ...
      return apiClient.deserialize(response.body, 'RealEstateVisit')
          as RealEstateVisit;
    } else {
      return null;
    }

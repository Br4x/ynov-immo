...
class RealEstateApi {
  ...
  RealEstateApi([ApiClient apiClient]) : apiClient = apiClient ?? defaultApiClient;

  /// get all real-estate by pagination
  ///
  /// 
  Future<List<RealEstatePagination>> realEstateGet({ List<String> where, String fields, String order, int offset, int limit }) async {
  ...
  Future<ApiResponse> realEstateIdDelete(int id) async {
    ...
    // create path and map variables
    String path = "/real-estate/{Id}".replaceAll("{format}","json").replaceAll("{" + "Id" + "}", id.toString());
  ...
  Future<RealEstate> realEstateIdGet(int id) async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
          apiClient.deserialize(response.body, 'RealEstate') as RealEstate ;
    } else {
      return null;
    }
  }
